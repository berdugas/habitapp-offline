...
### Up next: S4 — onboarding completion (Shrink, Cue, Worst-day, Confirmation + first Focus habit)

The second half of the becoming bridge. Adds four screens under `app/(onboarding)/` (shrink, cue, worst-day, confirm), wires the Continue button on Daily Action, and creates the first Focus habit on confirmation. See `docs/sprint_tickets/sprint-3-tickets.md` §0 for S4 field definitions already stubbed in `OnboardingDraft`.
